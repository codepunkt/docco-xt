...
(function() {
  ...
  write = function(source, sections, config) {
    var dest, destination, first, hasTitle, html, title, toOutput;
    destination = function(file) {
      var relative;
      if (config.basedir) {
        relative = path.dirname(path.relative(config.basedir, file));
        return path.join(config.output, relative, path.basename(file, path.extname(file)) + '.html');
      } else {
    ...
    source = config.basedir ? path.relative(config.basedir, source) : source;
    console.log("docco: " + (path.basename(source)) + " -> " + dest);
    return fs.outputFileSync(dest, html);
  };
  ...
  run = function(args) {
    ...
    c = defaults;
    commander.version(version).usage('[options] files').option('-L, --languages [file]', 'use a custom languages.json', _.compose(JSON.parse, fs.readFileSync)).option('-l, --layout [name]', 'choose a layout (parallel or linear)', c.layout).option('-o, --output [path]', 'output to a given folder', c.output).option('-c, --css [file]', 'use a custom css file', c.css).option('-t, --template [file]', 'use a custom .jst template', c.template).option('-b, --basedir [path]', 'use a given folder as base directory for relative paths', c.basedir).option('-e, --extension [ext]', 'assume a file extension for all inputs', c.extension).parse(args).name = "docco";
